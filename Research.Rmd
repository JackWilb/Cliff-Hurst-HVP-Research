...
#pcacomb = prcomp(comb[1:41], scale = TRUE)
#pcacomb$rotation[1:41,1:7]
glm = glm(type ~ ., data = entsenman[,c(31:41,42)], family = "binomial")
tidy(glm)
```
...
```{r}
corMat = cor(entreps[1:41])
factors = fa(r = corMat, nfactors = 1, n.obs = nrow(entreps), rotate = "promax")
factors
```
